function [sol,power, dCorr]=MatchingTest(dis,dim,tran,tesn,reps,options)
...
elseif nargin < 5
    options = struct('nonlinear',0,'match',2,'neighborSize',max(10,dim(length(dim))+5),'jointSelection',1,'weight',1,'scaling',1,'numData',2,'maxIter',-1,'oos',0,'permutation',0);
end
...
if ~isfield(options,'jointSelection')
    options.jointSelection = 1;
end
if ~isfield(options,'neighborSize')
    options.neighborSize = max(10,dim(length(dim))+5);
end
...
%sol=zeros(dim(1),n*numData,3,reps);
dCorr=zeros(reps,1);
%Start testing loop
index=[];
for r=1:reps
   %r
    ...
    [disO,~,~]=GetRealData(dis,0,tran,tesn,options);
    try
        if options.match==3
            options1.match=1;
            for dind=1:length(dim)
                [sol, dCorr(r)]=ManifoldMatching(disO,dim(dind),options1);
                disO=[squareform(pdist(sol(:, 1:n)')) squareform(pdist(sol(:, n+1:2*n)'))];
            end
            power(:,r,3)=plotPower(sol,numData,tesn,div);
        else
            for j=1:options.match+1
                ...
                power(:,r,j)=plotPower(sol,numData,tesn,div);
            end
        end
    catch
        index = [index r];
