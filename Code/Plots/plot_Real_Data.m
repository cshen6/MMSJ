function plot_Real_Data(matchingMethod)
...
%     switch i
%         case 1
            fileName=strcat(rootDir,'Data/Results/Brain',num2str(matchingMethod),'Results.mat');
            xla='Number of Training Data';
            figureName='Brain';
            tit='Brain Structural Networks';
            lgdLoc='NorthWest';
%         case 2
...
%     end
    ...
    set(gca,'FontSize',21);
    set(gca,'XTick',[1,9,18],'XTickLabel',[50,75,100],'YTick',[0.5,1]); 
    saveas(ax,fullfile(strcat(rootDir,'Figures/'), strcat(figureName,'Acc',num2str(matchingMethod))),'png')
    
%     ax=figure;
%     plot(x,powerMMSJ,'.-', x,powerMDS,'.:', x,powerISO,'.--',x,powerLLE,'.--',x,powerLTSA,'.--', 'LineWidth',4);
%     legend('MMSJ', 'MDS','Isomap', 'LLE', 'LTSA','Location',lgdLoc);
...
%     xlim([1 length(numRange)])
%     set(gca,'FontSize',25);
%     set(gca,'XTick',[1,9,18],'XTickLabel',[50,75,100],'YTick',[0,0.5,1]); 
